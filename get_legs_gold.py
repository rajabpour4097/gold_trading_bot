from metatrader5_config_gold import TRADING_CONFIG
...
def get_legs(data, custom_threshold=None, verbose: bool=False):
    ...
    ##################                ###############################################################   
        ...
        # برای طلا: threshold بر حسب دلار است، نه پیپ
        # بنابراین نیازی به ضرب در 10000 نیست
...
def custom_price_diff(data, j, current_price=0, legs=[]):
    ...
    if legs[j-2]['direction'] == 'up':
        price_diff = abs(current_price - row['high'])
        return price_diff
    else:
        price_diff = abs(current_price - row['low'])
        return price_diff
